...
项目自带一个轻量网关（`server/index.js`）：负责提供前端静态页面，并把 `/v1/*` 反代到上游 `sora2api`，在服务端注入 `SORA2API_KEY`。
...
开启后，未登录用户请求 `/v1/*` 会返回 `401`，前端会弹出登录框；登录后使用 Cookie 保持会话。

## Docker 部署（推荐配合 OpenResty 反代）

该项目在 Docker 中会：
- 构建前端到 `dist/`
- 运行内置网关（Node）监听 `18130`，统一转发 `/v1/*` 到上游 `sora2api`

1) 准备 `.env`（不要提交到 Git）

```bash
cp .env.example .env
# 编辑 .env：至少设置 SORA2API_BASE_URL / SORA2API_KEY
```
